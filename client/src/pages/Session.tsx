...
const SessionControls = () => {
  ...
  return (
    <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-4 bg-slate-900/90 p-3 rounded-2xl backdrop-blur-md border border-slate-700/50 shadow-2xl z-20 transition-all">
      <button 
        type="button"
        onClick={toggleMic}
...
const Session: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchToken = async () => {
      try {
        ...
        if (mockRole) {
          ...
          // Grab Firebase ID Token dynamically
          const currentUser = auth.currentUser;
          if (currentUser) {
            const idToken = await currentUser.getIdToken(true);
            authHeader = `Bearer ${idToken}`;
          } else {
             // Fallback wait if rendering unmounted or auth state not fully warmed
        ...
        // Extended significantly to cover the deepest Free-Tier Supabase cold starts
        ...
        if (response.ok && data.token) {
          ...
          if (!socket.connected) {
             ...
             await new Promise<void>(resolve => {
                ...
                setTimeout(handler, 2000); // Max wait
             });
          }

          // Send authenticated join event after we're verified
    ...
    if (isTeacher) {
      ...
      const handleAttentionUpdate = (data: any) => {
        if (data.studentId === myIdentity) {
          setMyScore(data.score);
        }
        setRoster(prev => {
          if (!prev[data.studentId]) return prev;
          return { ...prev, [data.studentId]: { ...prev[data.studentId], score: data.score } };
        });
      };
      ...
      const handleSessionEnded = () => {
        navigate(isTeacher ? '/dashboard' : '/portal');
      };
  ...
  }, [code, role, user, isTeacher, myIdentity, navigate]);
  ...
  const handleEndSession = () => {
    if (window.confirm('Are you sure you want to end this session for everyone?')) {
      ...
      // Fallback timer if socket is dead
      setTimeout(() => {
        if (isEnding) {
  ...
  return (
    ...
    >
        ...
        <header className="px-6 py-4 flex justify-between items-center bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 shadow-sm z-30 transition-colors rounded-2xl mb-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button 
              ...
              onClick={isTeacher ? handleEndSession : () => navigate('/portal')}
              disabled={isEnding}
            >
            {isEnding ? (
              <>
      ...
      {/* Sidebar Area (Warning Panel for students / Monitoring for teachers) */}
      <div className="w-full md:w-80 border-t md:border-t-0 md:border-l border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-col transition-colors">
        {isTeacher ? (
          <div className="p-4 flex-1 overflow-y-auto">
            ...
            {/* Teacher Dashboard Sidebar List */}
            <div className="space-y-3">
              {Object.values(roster).length === 0 ? (
                ...
                // Add special border for phone detection
